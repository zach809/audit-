...
function functionBlocks(source: string): FunctionBlock[] {
  ...
  let open: { name: string; exported: boolean; firstLine: number } | null = null;
  lines.forEach((line, index) => {
    const start = /^(export\s+)?(?:async\s+)?function\s+([A-Za-z0-9_]+)/.exec(line);
    if (start && !open) {
      open = { name: start[2], exported: Boolean(start[1]), firstLine: index };
      return;
    }
    if (open && line === "}") {
      blocks.push({ ...open, body: lines.slice(open.firstLine, index + 1).join("\n"), lastLine: index });
      open = null;
    }
  });
  return blocks;
...
describe("non-production deployments do not write", () => {
  ...
  it("fails when a new write route or server action skips the guard", () => {
    ...
    assert.ok(direct.size >= 10, `the sweep stopped seeing the data layer, found ${direct.size} writers`);
    ...
    for (const file of libFiles()) {
      ...
      for (const line of mutatingLineNumbers(blankDataLiterals(source))) {
        assert.ok(
          blocks.some((block) => line >= block.firstLine && line <= block.lastLine),
    ...
    const referencesAWriter = (source: string) => [...reachableExported].some((writer) => new RegExp(`\\b${writer}\\b`).test(source));
    const checked: string[] = [];
    for (const file of appFiles((name) => name === "route.ts" || name === "actions.ts")) {
      const source = readFileSync(file, "utf8");
      if (!referencesAWriter(source) && mutatingLineNumbers(source).length === 0) continue;
      checked.push(file);
    ...
    assert.ok(checked.length >= 9, `expected at least nine writing entry points, found ${checked.length}`);
    for (const entry of ["api/reviews/route.ts", "api/post-closure/sync/route.ts", "api/post-closure/followups/route.ts", "api/auth/clio/callback/route.ts", "actions.ts"]) {
      assert.ok(checked.some((file) => file.endsWith(entry)), `the sweep no longer sees ${entry} as a write path`);
    }
